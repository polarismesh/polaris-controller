...
import (
	...
	"github.com/polarismesh/polaris-controller/pkg/inject/istio/pilot/cmd/pilot-agent/status"
	"github.com/polarismesh/polaris-controller/pkg/inject/istio/pilot/pkg/model"
	"github.com/polarismesh/polaris-controller/pkg/inject/istio/pkg/config/mesh"
...
func loadConfig(injectFile, meshFile, valuesFile string, env *model.Environment) (*Config, *meshconfig.MeshConfig, string, error) {
	...
	klog.Infof("New inject configuration: sha256sum %x", sha256.Sum256(data))
	klog.Infof("Policy: %v", c.Policy)
	klog.Infof("AlwaysInjectSelector: %v", c.AlwaysInjectSelector)
	klog.Infof("NeverInjectSelector: %v", c.NeverInjectSelector)
...
func (wh *Webhook) addContainer(pod *corev1.Pod, target, added []corev1.Container, basePath string) (patch []rfc6902PatchOperation) {
...
func (wh *Webhook) createPatch(pod *corev1.Pod, prevStatus *SidecarInjectionStatus, annotations map[string]string, sic *SidecarInjectionSpec,
	...
	addAppProberCmd := func() {
		...
		if sidecar == nil {
			klog.Errorf("sidecar not found in the template, skip addAppProberCmd")
			return
		}
	...
	patch = append(patch, wh.addContainer(pod, pod.Spec.InitContainers, sic.InitContainers, "/spec/initContainers")...)
	patch = append(patch, wh.addContainer(pod, pod.Spec.Containers, sic.Containers, "/spec/containers")...)
...
func (wh *Webhook) inject(ar *v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	patchBytes, err := wh.createPatch(&pod, injectionStatus(&pod), annotations, spec, deployMeta.Name)
	if err != nil {
